...
3. **§7 — plugins declarativos**: F1–F7 implementadas. Pendiente F8 (plugin commerce real end-to-end).
...
### Siguiente objetivo: §7 F8 (plugin `commerce` real end-to-end)

F7 ya está: en resume, si el manifest del FS sube de versión (mismo `name`, hash distinto), `bindPluginsForResume` ejecuta la cadena `migrate[v]` para convertir el slice del save al shape nuevo en vez de abortar (las fixtures de la versión nueva ya las valida `loadGamePluginManifests` al cargar). Slice-only y fail-loud ante huecos en la cadena, degradación o cambio sin bump. Falta F8: materializar un plugin `commerce` v1 completo (manifest shipped + ciclo `trade_offered`→`trade_completed` end-to-end vía el bridge) como prueba de concepto, y documentar el patrón para que el motor narrativo lo replique. Pendiente menor de F7: la evolución en **runtime** (un `plugin_register` con `version` mayor que reemplace al plugin vigente con su `migrate`) — hoy `plugin_register` añadiría un segundo record coexistente; la migración sólo opera en resume.
...
> **Estado 2026-06-23 (4ª pasada)**: **F1–F7 implementadas** (F1–F4 en `plugins/f1-f4`; F5 en `plugins/f5-plugin-register`: `registerRuntimePlugin` con fixtures obligatorias para génesis runtime, endpoints `/plugins` y `/plugins/register` en el state API, tools MCP `plugin_register`/`plugin_list`, manifest embebido en el save y rebind en resume; F6: `serializeForLlm()` proyecta `derived_views` al contexto, tool `plugin_inspect` + endpoint `GET /plugins/{id}/inspect`; F7: migración de slice en resume vía `migrate[v]` cuando el FS sube de versión, en `plugins/f7-migrate`). 263 tests en verde + smokes reales (registro por HTTP, 400 con motivo, resume con manifest embebido, `inspect` por HTTP con vista/slice y 400 por id/vista inexistente, migración v1→v2 vía loader de FS con re-resume idempotente). Pendiente F8.
>
