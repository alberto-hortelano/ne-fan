...
3. ~~**§7 — plugins declarativos**~~ — **CERRADO** (2026-08-23, issue #164): F1–F8 más la evolución en runtime. `plugin_register` con el mismo `name` y versión mayor migra el slice y SUSTITUYE el record (`action: "migrated"`), el mismo manifest es un no-op (`unchanged`) y una versión igual o menor se rechaza con el MISMO texto que el resume — la cadena vive en `src/plugins/migrate.ts` y `arch-rules.json` no deja que aparezca una segunda.
...
> - F7: la migración en resume es **slice-only** (`runMigrationStep` aplica `migrate[v]` con WriteAuth vacía ⇒ escribir fuera de slice o emitir eventos lanza; re-aplicarlo en cada resume doblaría efectos externos). La guarda se duplica estáticamente en `validateManifestStatic` (rechaza migrate con path externo o `emit_event`). El record migrado adopta el id/version/slice nuevos preservando `name` y `activated_at`, así el siguiente resume casa por id sin re-migrar (idempotente). El `origin` lo pone el manifest que pasa a gobernar el record: en resume por FS sigue siendo el mismo (el manifest sigue en disco), y en un registro de runtime pasa a ser el del motor, porque el manifest queda embebido en el save y el JSON del disco deja de mandar. La misma cadena sirve a `plugin_register` desde 2026-08-23 (#164): vive en `src/plugins/migrate.ts` y es lo único que puede llamar a `runMigrationStep` (regla `cadena-de-migracion-unica`).
...
- **Slices grandes en saves**: un plugin mal diseñado puede crecer sin cota (ej. `transactions[]` que acumula todo el histórico). Sin mitigación implementada: el campo de manifest que iba a declarar el tamaño esperado existió en el schema sin que ningún código lo aplicara y se retiró en #368-F6 (2026-09-02). Si el problema aparece, la respuesta es un aviso del bridge medido sobre el slice real, no un hint que el autor del manifest rellena a ojo.
