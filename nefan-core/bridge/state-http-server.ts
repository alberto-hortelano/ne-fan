    ...
    dispatchStateRequest(ctx, {
      ...
      .then(async (result) => {
        if (result.mutated) {
          ...
          // El body nombra el estado exacto —aplicado, no guardado— porque
          // reintentar a ciegas duplicaría la mutación.
          try {
            await onMutation();
          } catch (err) {
            ...
            sendJson(res, 500, {
              ok: false,
              error: `aplicado en memoria pero NO guardado: ${motivo}`,
