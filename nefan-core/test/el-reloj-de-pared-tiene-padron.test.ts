...
describe("el reloj de pared tiene padrón (#711): qa/guiones/**", () => {
  ...
  it("el censo de hoy: 29 relojes en 10 guiones, y ningún callback de rAF con parámetro", () => {
    ...
    // sobre #711: sus cuatro t0/Δ de Node cada uno. El 07 salió el 2026-09-24
    // (#734, tanda AY): su sello `t: Date.now()` no lo leía ningún aserto. Si
    // esta cifra cambia con el padrón al día, se actualiza aquí.
    const ocupados = [...censo].filter(([, rs]) => rs.length > 0);
    assert.deepEqual(
      ocupados.map(([f]) => f.replace(/^qa\/guiones\/(\d+)-.*$/, "$1")),
      ["10", "131", "133", "157", "164", "166", "168", "170", "171", "172"],
    );
    assert.equal(ocupados.reduce((a, [, rs]) => a + rs.length, 0), 29);
