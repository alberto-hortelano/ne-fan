...
/** Pulsa las nueve entradas de juego que #285 nombra. Teclado y ratón REALES
 *  (CDP), nunca `inputDriver`: el driver programático escribe en el provider
 *  por debajo de `window`, así que con él la puerta no está en el camino y el
 *  guion mediría el vacío. */
async function lasNueveEntradas(ctx) {
  ...
  for (const tecla of ["w", "a", "s", "d", "Shift", "ArrowLeft", "ArrowUp"]) {
    await k.down(tecla);
    try {
...
export default async function (ctx) {
  ...
  const opcion = await ctx.page.evaluate(() => {
  ...
  ctx.expect(
    "el panel de dev ofrece alguna fixture que cargar con el título delante",
    Boolean(opcion),
  ...
  const movimiento = antesControl.pos !== despuesControl.pos;
  const combate =
  ...
  ctx.expect(
    "CONTROL: cerrado el título, esas mismas entradas SÍ responden (movimiento, ataque y teclas dev)",
    movimiento && combate && dev,
    `movimiento=${movimiento} (${antesControl.pos}→${despuesControl.pos}) · ` +
