...
| `134-la-esquina-del-edificio-no-se-corta` | **#601, escrito por QA al validar la PR 1**: el arreglo de la esquina vivía entero en `nefan-core` y en el BANCO no lo candaba nadie — el 81 y el 91 sondean centro y bordes de un spawn y andan **de frente**, el 02/45/06 empujan contra el muro del plan también de frente, y la diagonal exacta contra una ESQUINA, que era la única puerta, no la caminaba ninguno. Carga la fixture `robledo_tile` del selector Room (sin motor, sin partida, **cero créditos**), DESCUBRE sondeando el rectángulo sólido de tres edificios del pueblo —desde un mirador a 28 m, porque `probeCollide` es `collidesAt` y a quien ya está dentro le contesta que no: es el H2 de #538 asomando en el instrumento— y ANDA contra cada una de sus cuatro esquinas por la diagonal exterior. Tres mitades, y las tres hacen falta: **no entra** (se espera POR EL FALLO y la expiración es el aserto, molde del 02), **pero anda** (sin esto «no entró» sale verde con el jugador parado en la salida) y el **CONTROL de frente** contra una cara, que se para pegado a la pared (sin él, el día que nada frenase los dos primeros saldrían igual de verdes midiendo campo abierto). **PROBADO EN NEGATIVO**: devolviendo `pasoDelJugador` a la resolución de la base, **10 de las 11 esquinas en ROJO** y el control de frente VERDE — el rojo es de la esquina, no de «dejó de frenar todo». Dato que sale de aquí: con el código de `243d5b3b` el jugador no cruzaba el edificio del PLAN, se quedaba DENTRO y dejaba de andar (6,1 m y parado en (−11,91, −11,91), dentro de la Casa del Concejo) |
