...
func _process(_delta: float) -> void:
	...
	if is_action:
		...
		if delta_xz.length() < 0.3 and delta_xz.length_squared() > 0.00001:
			...
			# Apply half delta — animation moves model by the same amount
			# so total displacement matches the animation's intended movement
			body.position.x += world_dx * 0.5
			body.position.z += world_dz * 0.5
