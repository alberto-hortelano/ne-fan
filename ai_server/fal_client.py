...
class FalFillClient:
    """Inpainting remoto para el PELADO por capas del proscenio (FLUX Fill en
    fal.run): imagen + máscara (blanco = hueco) + prompt de lo declarado
    detrás → imagen completa con el hueco continuado. Validado en el
    experimento lateral de julio (~$0.03/relleno; behind_labels funciona,
    ...
    def _data_uri(self, png: bytes) -> str:
        return "data:image/png;base64," + base64.b64encode(png).decode()

    def fill(self, image_png: bytes, mask_png: bytes, prompt: str, timeout: float = 180.0) -> bytes:
        """Devuelve la IMAGEN COMPLETA con la zona de la máscara rellenada."""
        payload = {
            "image_url": self._data_uri(image_png),
            "mask_url": self._data_uri(mask_png),
            "prompt": prompt,
            "num_images": 1,
            "output_format": "png",
            "seed": 7,
        ...
        with httpx.Client(timeout=timeout) as client:
            resp = client.post(f"{self.BASE_URL}/{self.model}", headers=self.headers, json=payload)
            resp.raise_for_status()
            data = resp.json()
        images = data.get("images") or []
        if not images or not images[0].get("url"):
            raise RuntimeError(f"fal fill: respuesta sin imagen ({list(data.keys())})")
        url = images[0]["url"]
        if url.startswith("data:"):
            _, _, b64 = url.partition(",")
            return base64.b64decode(b64)
