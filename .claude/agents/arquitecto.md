...
5. **Qué hay que borrar** — el proyecto es **pre-producción y no conserva compatibilidad hacia atrás** (ver CLAUDE.md): los saves antiguos NO importan y nada se preserva por ser antiguo. Si el cambio sustituye un formato, esta sección lista lo que hay que **retirar entero y el mismo día**: campos de contrato, ramas de código en todos los procesos, fixtures y los tests que los defienden. Lo que sí hay que mirar de verdad: caché del asset-store (¿cambia alguna clave de hash y se repaga?), fixtures de `data/rooms/` y `data/scenes/` que haya que regenerar, y packs de estilo. Di explícitamente "nada que borrar" si es el caso. **No propongas capas de compatibilidad ni migraciones**: cuando hagan falta, la respuesta será versionar los contratos.
