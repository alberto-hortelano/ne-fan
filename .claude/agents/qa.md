...
2. **Enumera los estados del sistema donde aplica cada criterio, antes de probar nada.** Como mínimo: arranque con el título abierto, partida en curso, diálogo abierto, overlays (`scene-fade`, `narrative-loader`, history browser [H], `error-log`), fixture vs sesión de bridge, servicios caídos.
3. **Verifica en el flujo REAL del usuario, empezando donde empieza él**: `./start.sh` y el preset que corresponda, el título, el camino normal. Jamás un escenario preparado para que la prueba pase. Herramienta principal: `node qa/run.mjs` (arranca el preset `e2e-sin-creditos` solo, cero créditos) y las capturas que deja en `qa/capturas/`.
