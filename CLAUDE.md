...
| `npm run ejercicio` (paso propio del job `nefan-core`, junto a `crap`) | que la batería de cada módulo **EJERCE** lo que ese módulo muta, y no solo lo carga. El agujero con su número: `place-target.ts` estaba en un módulo cuya batería lo importaba en tres tests y **no lo llamaba en ninguno** — 35 mutantes, 35 vivos, que no es deuda de test sino MEDIDA QUE NO EXISTE (#598a). **No se mide con `lcov`**, que da un falso NEGATIVO justo ahí (daba 25 de 39 líneas «cubiertas», y las cubiertas eran el cuerpo de una función llamada CERO veces): se cuentan **invocaciones** con `NODE_V8_COVERAGE`. Fuera del bucle interno de `verify` por coste medido (~60 s contra los ~13 s que el usuario decidió no encarecer) |
...
| `nefan-core/data/contract/sondas-de-movimiento.json` (+ `test/la-consulta-de-movimiento-tiene-dueno.test.ts`) | que el banco pregunte por el PUNTO cuando quiere el punto. `probeCollide` es consulta de MOVIMIENTO y lee la posición viva como origen, así que `collidesAt(p)` con el sujeto EN `p` vale **siempre `false`** — once asertos del banco no podían ponerse rojos (#644, #662). Cada consulta viva está en el padrón con su motivo o el test se pone rojo, y **cuenta SITIOS DE USO alcanzables** —las llamadas que cuelgan de un alias, resueltas en el árbol de sintaxis— y no nodos con el nombre: el censo por grafía nace ciego ante un alias, y contar nodos dejaba pasar N llamadas detrás de UN `pc` (#686). El padrón declara además `_lo_que_esto_NO_sujeta`, obligatorio en su zod, con un unitario que MIDE esos agujeros en vez de prometer que no existen |
| `nefan-core/data/contract/saltos-sin-observar.json` (+ `test/un-salto-del-guion-se-observa.test.ts`) | que un guion no se salte un bloque sin que nadie lo vea (#356), en las formas que el detector enumera: en el cuerpo de un guion, un `return` (tras un `if`, `case` o `catch`, o sin guarda) o un `if` con una rama muda que dejan asertos detrás —propios o de helpers— están OBSERVADOS (la rama declara, afirma o lanza, o la condición se afirmó antes) o en el padrón con su motivo. Por el árbol y en `npm test`, porque la batería de navegador no corre en CI. NO es «todo salto»: lo que no ve (funciones anidadas, bucles, cortocircuito, polaridad…) está en `_lo_que_esto_NO_sujeta`, cada punto medido |
| `ci.yml` job `candados-headless` (+ `data/contract/banco-medido.json`) | que los candados en negativo de `qa/` que no abren navegador corren en cada PR: el 05-09 uno llevaba un día rojo en `main` sin que nadie lo supiera, porque nadie lo corría. Y que el banco tiene quien lo mire: todo `qa/lib/*.mjs` lo importa un test o está eximido con motivo (dirección test → banco; `qa/` nunca entra en producción, regla `el-banco-no-entra-en-produccion`). Y que **el banco es `.mjs` y solo `.mjs`**: bajo `qa/` solo caben las extensiones de una lista blanca, porque Node 24 ejecuta un `.ts` sin tsx y todo lo que decide «qué es el banco» filtra por `.mjs`; el barrido es uno solo (`test/banco-ficheros.ts`) y comparte saltos con el padrón de sondas (#686). La batería de navegador (`qa/run.mjs` y `bateria-`/`esperas-`) sigue siendo corrida local; qué entra y qué no, en `qa/README.md` |
...
| `traer [run-id]` | el coordinador | **VACÍA `reports/mutation/`** y baja el artefacto; rechaza la descarga a la que le falte un informe, le sobre uno, o traiga uno cuyo **sello no case** con el que midió la corrida. Lo que vacía es la BASE de la comparación —los `Timeout` y los `NoCoverage` solo viven en los informes, la huella no los guarda—, así que **se NIEGA si hay informes** y dice las dos salidas: `mv reports/mutation reports/mutation-base`, o `--sobrescribir` |
| `repartir [--comentar] [--instrumento-nuevo]` | el coordinador | delta contra la huella de HEAD, atribución honesta —anclada en el `desde` que trae la corrida, no en el tag que ella misma mueve— y comentario en la PR de origen; repite el guardia del sello. **Escribe la huella**: no sirve para juzgar un instrumento nuevo, porque destruye la medida contra la que había que juzgarlo. Y **se NIEGA** si algún fichero trae MENOS mutantes con veredicto que la medida anterior sin que su fuente haya cambiado: eso no lo puede hacer una PR, es el instrumento midiendo menos, y las muertes perdidas salen del numerador Y del denominador, así que ningún `break` se entera (#596). Solo esa dirección —un denominador que CRECE también es el instrumento, pero midiendo MÁS, y se dice sin bloquear—. `--instrumento-nuevo` declara que la pérdida está aceptada a propósito; sin la bandera, primero se mira EN SECO con `comparar` |
| `comparar [--timeouts <dir>]` | el coordinador | el MISMO delta EN SECO: mira y no toca (ni huella, ni tag, ni comentario, ni `reports/`). Sale con ≠ 0 salvo que se cumplan **las SIETE**: 0 nuevos · 0 resueltos · **0 incomparables** · 0 sin base · **comparables > 0** · **0 sin medir y la corrida mueve el tag** · **0 sin ejercer**. Las tres en negrita son las que parecen adorno: un fichero incomparable devuelve nuevos y resueltos vacíos (pedir solo «0 y 0» se cumpliría sin comparar nada); un módulo suelto daba «SE PUEDE ADOPTAR» sobre 1 fichero de 87; y un `Survived` que pasa a `NoCoverage` es invisible al delta porque `esVivo` los colapsa — o sea un instrumento que mide MENOS presentado como idéntico. Los mutantes que clasifica el RELOJ van en bloque aparte y **no** se restan del veredicto |
...
|--------|------|-----------|--------|
| 1 · Play | `play` | asset-store + remote-gen + sprite-forge + bridge + narrative-mcp + ai_server + pausa Claude Code + cliente | Sesión narrativa completa — con Imagen IA solo restaura lo pagado; genera (y GASTA) con `NEFAN_ENTORNO=produccion` |
| 2 · Cliente web (dev) | `cliente-web` | bridge + asset-store + remote-gen + sprite-forge + cliente | Iterar UI y renderer sin motor narrativo — solo gasta con Imagen IA y `NEFAN_ENTORNO=produccion` |
...
```json
{
  ...
  "terrain_grid": { "grid": ["..."], "solid_chars": ["w"] },
  "objects": [
