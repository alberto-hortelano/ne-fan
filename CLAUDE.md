...
| `comparar [--timeouts <dir>]` | el coordinador | el MISMO delta EN SECO: mira y no toca (ni huella, ni tag, ni comentario). Sale con ≠ 0 salvo que se cumpla TODO: 0 nuevos, 0 resueltos, **0 incomparables**, 0 sin base y **comparables > 0** — un fichero incomparable devuelve nuevos y resueltos vacíos, así que pedir solo «0 y 0» se cumpliría sin haber comparado nada. Los mutantes que clasifica el RELOJ van en bloque aparte y no se restan del veredicto |
