...
- **Asset library indexada** — el asset-store (:8767, `cache/manifest.sqlite3`) traquea las superficies pintadas con su prompt; es el ÚNICO kind del índice desde #257 (los siete sin productor se archivaron en `archivo/cache/` con sus filas en `manifest-retirado.json`, y el store se niega a arrancar si reaparece otro: `scripts/manifest-solo-surface.ts`). Claude recibe la librería en cada request narrativa y reusa por DESCRIPCIÓN verbatim, no por hash.
