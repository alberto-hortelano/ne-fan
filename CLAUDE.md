...
| `nefan-core/data/contract/saltos-sin-observar.json` (+ `test/un-salto-del-guion-se-observa.test.ts`) | que un guion no se salte un bloque sin que nadie lo vea (#356): todo `return` temprano o `if` con una rama muda en el cuerpo de un guion está OBSERVADO —la rama declara (`sinMedirBloque`), afirma o lanza, o la condición se afirmó antes— o está en el padrón con su motivo. Por el árbol y en `npm test`, porque la batería de navegador no corre en CI. Trinquete que solo encoge y `_lo_que_esto_NO_sujeta` medido |
