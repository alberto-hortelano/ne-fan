...
| `nefan-core/data/contract/sondas-de-movimiento.json` (+ `test/la-consulta-de-movimiento-tiene-dueno.test.ts`) | que el banco pregunte por el PUNTO cuando quiere el punto. `probeCollide` es consulta de MOVIMIENTO y lee la posición viva como origen, así que `collidesAt(p)` con el sujeto EN `p` vale **siempre `false`** — once asertos del banco no podían ponerse rojos (#644, #662). Cada aparición viva está en el padrón con su motivo o el test se pone rojo, y **cuenta NODOS del árbol**, no coincidencias de texto: el censo por grafía nace ciego ante un alias o un nombre partido. El padrón declara además `_lo_que_esto_NO_sujeta`, obligatorio en su zod, con un unitario que MIDE esos agujeros en vez de prometer que no existen |
