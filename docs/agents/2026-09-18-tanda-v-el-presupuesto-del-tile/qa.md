...
# Vuelta 2 — re-verificación de H-1/H-3/H-4/H-5 y del 63 con la espera de AF (2026-09-21)

Sobre HEAD `4cdec765` (rebasada sobre `main` = `39421b12`; el guion de medida es ahora el **157** y el de
QA el **159**; PR #708 abierta). Solo lo afectado; línea base 56/56 en los dos tests de la tanda,
md5 de los cinco ficheros tocados tomados antes y comprobados después de cada sabotaje. Máquina en
reposo (load 1,8–2,0).

| Qué | Veredicto | Evidencia |
|---|---|---|
| **H-1** · el test de equivalencia exige la lectura ENTERA (`deepEqual`) | ✅ cerrado | Sonda reescrita con la cascada vieja (llegado > fallo > rechazado, cada rama solo la señal ganadora) → **43/45**, rojos exactamente «llegado … devuelve la lectura ENTERA» y «rechazado gana a fallo … devuelve la lectura ENTERA». Antes de la vuelta esa misma cascada daba 45/45 |
| **H-3** · el padrón tiene 15 (13 + los 2 del default) | ✅ cerrado, con un sitio NUEVO que llegó de `main` (abajo) | Las 13 entradas: 05, 42, 63 (`porRondasHastaRechazo`), 08, 09, 15, 74, 75, 144, **49:147, 49:164, 60:213, 65:115**. Mi detector por el árbol (`scratchpad/detector.mjs`: toda espera de pared de `qa/**` cuyo desc/predicado huele a tile — `currentTile`, `tiles.includes`, `scenes_loaded`, «otro tile», «destino», `viaje`…) devuelve **13 en padrón** y, fuera, solo esperas de otro sujeto (atlas texturado del 60, fixtures a 20–60 s, propuestas de frontera del 109/58 en `expectEspera`) **más una del MISMO sujeto: `154:128`** (ver H-3′). Las once «la escena vuelve tras reanudar» (180 s) siguen fuera y el detector no las toca (su predicado es `status().scene`, un resume) |
| **H-3′ · importante — `qa/guiones/154-ir-y-volver-no-repone-a-los-npc-del-tile.mjs:128`** | ❌ sitio del sujeto fuera del padrón | `esperarLlegada(ctx, tileAnterior, desc)` → `ctx.waitFor(desc, … currentTile !== anterior … viaje …, 240_000)`, calcado del 09 («misma espera que el guion 09») y con el mismo `runTileGeneration` detrás. Entró en `main` con #699 (`243fcf9f`) **después** del censo de V y llegó con el rebase: es el agujero declarado `_lo_que_esto_NO_sujeta[0]` con un sujeto vivo, el mismo día. No es regresión de V, pero la PR va a fusionar un árbol donde «UN cortafuegos para las esperas del tile» tiene una decimosexta espera a 240 s. Pide una línea (`240_000 → MS_DEL_TILE` + import) y una entrada más en el padrón (el test fija 13 → 14) |
| **H-3″ · menor — el motivo de dejar fuera las «vuelve tras reanudar» no está en el árbol** | ⚠️ | El coordinador pide «fuera con motivo escrito». El motivo («el sujeto es un resume, no un tile») está en el **mensaje de commit** de `4622c284` y en `implementacion.md` (gitignored); `grep -n reanudar` sobre `esperas-de-tile.json`, el test y `qa/README.md` = 0 (solo el nombre de fichero del 60). Quien lea el padrón mañana verá 13 viajes a `MS_DEL_TILE` y once esperas de 180 s del título sin saber por qué no son lo mismo. Una frase en `_comment` o en `_lo_que_esto_NO_sujeta` |
| **H-4** · clase `alias`, roja también en lectores | ✅ cerrado | Lector de mentira registrado con `const doble = MS_DEL_TILE * 2; waitFor(…, doble)` → ✘ (b) «lo BAUTIZA con otro nombre (`doble = MS_DEL_TILE * 2`)» (antes de la vuelta: 10/10 verde). El agujero restante está **declarado** en `_lo_que_esto_NO_sujeta[4]` («objeto, array o retorno de una función») y lo **medí**: lector con `const cfg = { ms: MS_DEL_TILE * 2 }; waitFor(…, cfg.ms)` → **11/11 verde**. Pero el coordinador dice «declarado con `it`» y **no hay `it` que lo mida**: el caso «un LECTOR no puede presupuestar por alias» mide el alias, no el objeto; la frase del JSON remite a ese caso como si lo cubriera. Es el mismo patrón que la tanda ya arregló para el literal (agujero con caso ejecutable): falta el `it` del objeto para que se ponga rojo el día que alguien lo cierre. Menor: hoy el único lector es el 157 y no lo hace |
| **H-5** · `90000` y `90_000` valen igual | ✅ cerrado | `90_000 → 90000` → **11/11 verde**; `90_000 → 91_000` → ✘ «declara `91_000` y el contrato dice 90000» |
| **63 con la espera de AF** (`porRondasHastaRechazo`, techo = `MS_DEL_TILE`) | ✅ | El padrón lee la cuarta forma (arg 3 del verbo; `desc: null`, casa por fichero + verbo). Sabotaje `63:140 MS_DEL_TILE → 60_000` → ✘ (a) «`porRondasHastaRechazo(∅)` (línea 136): presupuesta con `60_000`». La `esperarEnElSave(…, RONDA_MS)` interior (2 s de cadencia) no es el cortafuegos y el detector la deja fuera, bien |
| Guiones | ✅ | `157` ×1: p50 160 · p95 172 · máx 181 ms → **524×**, 32/32, +32 del motor, 10 s. `63` ×1: verde, 13 s. `159` (sin navegador): 6/6 ✔ |

**Veredicto de la vuelta**: los cuatro hallazgos están cerrados y cada cierre se pone rojo con su
sabotaje. Sigue **apto con reservas**, y las reservas cambian de nombre: **H-3′** (el 154 llegó de
`main` con la misma espera a 240 s: una línea y una entrada, antes de fusionar o como primera tarea
después), **H-3″** (el porqué de las once de «reanudar» solo vive en un commit) y el `it` que falta
para el agujero objeto/array/retorno de H-4. Ninguna hace falso lo entregado.
