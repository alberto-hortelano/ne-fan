# Requisitos — tanda AQ: el rechazo de intake llega a los clientes WS del banco (#694)
...
## Nota del coordinador para AQ

Esta carpeta es la de la tanda **AQ**, que hace SOLO **#694** (helper «mando un frame y espero SU respuesta» en `qa/lib/cable.mjs`, rechazo de intake reconocido por `kind:"protocolo"`, los 15 `una-respuesta` del padrón `clientes-ws-del-banco.json` a cero, sabotaje en el guion 152, `run.mjs:866` migrado o eximido con motivo). Lo de #693 es de la tanda AP y no se toca aquí. La crítica que produjo este reencuadre está en `critica-compartida-con-ap.md` (una sola crítica para las dos). Worktree `/home/al/code/ne-fan-tanda-aq`, rama `feature/tanda-aq`. Guion reservado AQ 171 (QA 172). Ficheros probables de roce con AP: ninguno según la crítica; si aparece alguno (p. ej. un guion que ambos migran), avísalo.
