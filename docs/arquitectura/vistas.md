...
arranca, cuál se encola (la MISMA clave ocupada se deduplica y se re-dispara
al quedar libre) y qué corrida sigue mandando (la vigencia es de la CLAVE: una
corrida de otro tile nunca desecha la de este, #729) lo decide
`PoliticaDeAtlas` (`src/scene/politica-de-atlas.ts`, con test y mutación): el
controller solo pregunta.
...
fichero y función: `normalizarFixture` en `world/fixtures-del-selector.ts`, las fixtures
