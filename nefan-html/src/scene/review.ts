 ...
 *
 *  Claude mira el plano base COMPUESTO del tile (ground + volumes) vía
 *  /review_scene_blueprint y devuelve fixes parciales; el plan corregido (o
 *  aprobado sin cambios) se re-aplica en local y se persiste al bridge con
 *  map_plan_update, que estampa `map_plan_reviewed` para que el resume no
 *  re-revise (y el plan quede idéntico ⇒ hit de caché de imagen). main.ts
 *  solo cablea las deps. */
 ...
 *  (ground/volumes) o ya revisados: no-op inmediato. */
export async function reviewTileBlueprint(key: string, deps: ReviewDeps): Promise<void> {
  ...
  if (!entry || !raw) return;
  const hasPlan = Array.isArray(raw.ground) || Array.isArray(raw.volumes);
  ...
  if (tc) {
    deps.reportMapPlan(tc.tx, tc.ty, {
      ground: review.fixes?.ground,
      volumes: review.fixes?.volumes,
    });
  }
  deps.log(
    review.fixes?.ground || review.fixes?.volumes
