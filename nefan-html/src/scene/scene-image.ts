...
export class SceneImageController {
  ...
  async analyzeSceneForTile(key: string): Promise<TileAnalysis> {
    ...
    try {
      ...
      // PROHIBIDO — NUNCA recortar la imagen pintada con las siluetas
      // DECLARADAS del compositor (modo "masks", probado y eliminado): el
      // modelo de imagen recoloca y reorienta lo declarado, así que una
      // máscara declarada recorta SUELO con la forma del objeto y el objeto
      // real queda cocido en la placa. No funciona y no va a funcionar. Los
      // recortes salen SIEMPRE de segmentar lo que el modelo PINTÓ (SAM2 +
      // visión, aquí debajo); lo declarado solo vale como PISTA (expected).
