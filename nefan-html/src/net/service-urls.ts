 ...
 *  - `?state=http://...` — la State API de ESE bridge; va con `?bridge=`.
...
function envFromQuery(search: string = location.search): Record<string, string | undefined> {
  ...
  return {
    ...
    // `?state=` — la State API del MISMO bridge que `?bridge=`.
    //
    // Existe porque `?bridge=` mueve el gateway y dejaba la State API donde
    // estaba, así que la página acababa preguntándole «¿con qué motor hablas?»
    // al bridge de al lado. Eso ahora se detecta y se rechaza (el `/health`
    // declara su `gateway_url`), pero rechazar sin dar salida convierte un
    // agujero en un callejón: quien mueve el bridge a propósito necesita poder
    // mover su State API con él.
    NEFAN_URL_WORLD_STATE: q.get("state") ?? undefined,
